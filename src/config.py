...
def get_new_config(current_config, new_config):
    return current_config if new_config is None else new_config
...
def combine_mlpconfigs(current_config, new_config):
    if current_config is None:
        ...
        return MLPConfig(
            input_size = get_new_config(current_config.input_size, new_config.input_size) ,
            output_size = get_new_config(current_config.output_size, new_config.output_size),
            hidden_sizes = get_new_config(current_config.hidden_sizes, new_config.hidden_sizes),
            is_binary_input = get_new_config(current_config.is_binary_input, new_config.is_binary_input),
            activations = get_new_config(current_config.activations, new_config.activations),
            dropouts = get_new_config(current_config.dropouts, new_config.dropouts),
            use_biases = get_new_config(current_config.use_biases, new_config.use_biases),
            use_batch_norms = get_new_config(current_config.use_batch_norms, new_config.use_batch_norms),
            use_layer_norms = get_new_config(current_config.use_layer_norms, new_config.use_layer_norms),
        )
...
def combine_fuserconfigs(current_config, new_config):
    if current_config is None:
        ...
        return FuserConfig(
            n_modality = get_new_config(current_config.n_modality, new_config.n_modality),
            method = get_new_config(current_config.method, new_config.method),
        )
...
def combine_configs(current_config, new_config):
    if current_config is None:
        ...
        return ModelConfig(
            ...
            n_batch=get_new_config(current_config.n_batch, new_config.n_batch),
